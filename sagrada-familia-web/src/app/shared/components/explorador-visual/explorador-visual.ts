...
export class ExploradorVisual {
  ...
  categoriasEstilizadas = computed<CategoriaEstilizada[]>(() =>
    this.categorias().map(cat => ({
      ...
      color: cat.color ?? 'bg-slate-100 text-slate-600',
  ...
  colorDeCategoria(nombre: string): string {
    return this.categoriaMapByName().get(nombre)?.color ?? 'bg-slate-100 text-slate-600';
  }

  claseTarjeta(nombre: string): string {
    const n = nombre.toLowerCase();
    if (n.includes('fruta'))                              return 'bg-orange-50 border-orange-100';
    if (n.includes('verdura'))                            return 'bg-green-50 border-green-100';
    if (n.includes('prote'))                              return 'bg-red-50 border-red-100';
    if (n.includes('cereal') || n.includes('tubérculo')) return 'bg-amber-50 border-amber-100';
    if (n.includes('lácteo') || n.includes('lacteo'))    return 'bg-blue-50 border-blue-100';
  ...
  private claseInactivaPor(nombre: string): string {
    const n = nombre.toLowerCase();
    if (n.includes('fruta'))                              return 'bg-white text-orange-600 border-slate-200 hover:border-orange-300 hover:bg-orange-50';
    if (n.includes('verdura'))                            return 'bg-white text-green-600 border-slate-200 hover:border-green-300 hover:bg-green-50';
    if (n.includes('prote'))                              return 'bg-white text-red-600 border-slate-200 hover:border-red-300 hover:bg-red-50';
    if (n.includes('cereal') || n.includes('tubérculo')) return 'bg-white text-amber-700 border-slate-200 hover:border-amber-300 hover:bg-amber-50';
    if (n.includes('lácteo') || n.includes('lacteo'))    return 'bg-white text-blue-600 border-slate-200 hover:border-blue-300 hover:bg-blue-50';
